...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    if (savedToken) {
      ...
      api.get('/usuarios/me') // <-- sem /api, pois já está no baseURL
  ...
  const handleRegister = async (username, email, password, steam_id = null) => {
    try {
      await api.post('/auth/register', { username, email, password, steam_id });
      return { success: true };
    } catch (error) {
  ...
  const value = { isAuthenticated, token, user, loading, login: handleLogin, register: handleRegister, logout };
